...
function convertTypeBoxCommandSchema(commandSchema, existingCommands = {}) {
  ...
  const unionVariants = Array.isArray(commandSchema.anyOf) ? commandSchema.anyOf : Array.isArray(commandSchema.oneOf) ? commandSchema.oneOf : null;
  const looksLikeSingleVariant = typeof commandSchema.properties === "object" && commandSchema.properties !== null && !Array.isArray(commandSchema.properties);
  const variants = unionVariants || (looksLikeSingleVariant ? [commandSchema] : null);

  if (!variants || variants.length === 0) {
    return {
      commands: null,
      error: "commandSchema is missing anyOf/oneOf command variants and does not look like a single command object schema",
    };
  }
  ...
  if (commandSchema !== undefined && commandSchema !== null) {
    ...
    if (!validationError) {
      ...
      if (convertedCommands) {
        ...
        warnings.push(`manifest.commands: commandSchema export is invalid (${validationError}). TypeBox conversion failed (${conversionError}). Skipping.`);
      }
    }
