...
`;
  ...
  it("parses GitHub remote URLs", () => {
    assert.equal(parseGitRemoteRepository("git@github.com:ubiquity-os/test-plugin.git"), "ubiquity-os/test-plugin");
    assert.equal(parseGitRemoteRepository("https://github.com/ubiquity-os/test-plugin.git"), "ubiquity-os/test-plugin");
    assert.equal(parseGitRemoteRepository("ssh://git@github.com/ubiquity-os/test-plugin.git"), "ubiquity-os/test-plugin");
    assert.equal(parseGitRemoteRepository("https://example.com/ubiquity-os/test-plugin.git"), null);
  });

  it("parses Deno timeline ref names", () => {
    assert.equal(parseDenoTimelineRefName("git-branch/feature-x", "main"), "feature-x");
    assert.equal(parseDenoTimelineRefName("production", "main"), "main");
    assert.equal(parseDenoTimelineRefName("preview/rev-123", "main"), "rev-123");
    assert.equal(parseDenoTimelineRefName("", "main"), "");
  });
});
...
describe("configuration resolution", () => {
  it("keeps positional local mode unchanged", async () => {
    const projectRoot = createProject("resolve-local");
    writeProjectFile(projectRoot, "package.json", `{"name":"test-plugin"}`);

    const originalArgv = process.argv.slice();
    process.argv = ["node", "plugin-manifest-tool.js", projectRoot];
    try {
  ...
  it("reads deno deploy settings from deno.jsonc", () => {
    const projectRoot = createProject("deno-config");
    writeProjectFile(
      projectRoot,
      "deno.jsonc",
      `{
  // generated by deno deploy switch
  "deploy": {
    "org": "ubiquity-os",
...
`
    ...
    assert.deepEqual(readDenoDeployConfig(projectRoot), {
      path: path.join(projectRoot, "deno.jsonc"),
      org: "ubiquity-os",
      app: "command-start-stop",
    });
  });

  it("resolves GitHub CI config from existing environment variables", async () => {
    const projectRoot = createProject("resolve-github-ci");
    await withTemporaryEnv(
      {
      ...
      async () => {
        ...
        assert.equal(config.projectRoot, projectRoot);
        assert.equal(config.manifestPath, path.join(projectRoot, "manifest.json"));
        assert.equal(config.repository, "ubiquity-os/test-plugin");
        assert.equal(config.refName, "feature/test");
      }
    );
  });

  it("resolves Deno context without GitHub env", async () => {
    const projectRoot = createProject("resolve-deno");
    writeProjectFile(
      projectRoot,
      "deno.jsonc",
      `{
    ...
    await withTemporaryEnv(
      {
        ...
        PLUGIN_MANIFEST_REPOSITORY: "ubiquity-os-marketplace/command-start-stop",
        PLUGIN_MANIFEST_PRODUCTION_BRANCH: "main",
        DENO_TIMELINE: "production",
      },
      async () => {
        const config = await resolveConfig();
        assert.equal(config.repository, "ubiquity-os-marketplace/command-start-stop");
        assert.equal(config.refName, "main");
        assert.deepEqual(config.denoDeploy, {
          path: path.join(projectRoot, "deno.jsonc"),
          org: "ubiquity-os",
          app: "command-start-stop",
        });
...
`
  ...
  it("falls back to git metadata when GitHub env and DENO_TIMELINE are absent", async () => {
    const projectRoot = createProject("resolve-git-fallback");
    writeProjectFile(projectRoot, "package.json", `{"name":"test-plugin"}`);
    runGit(projectRoot, ["init", "-b", "feature/fallback"]);
    runGit(projectRoot, ["config", "user.email", "codex@example.com"]);
    runGit(projectRoot, ["config", "user.name", "Codex"]);
    runGit(projectRoot, ["remote", "add", "origin", "git@github.com:ubiquity-os/test-plugin.git"]);
    ...
    await withTemporaryEnv(
      ...
      async () => {
        const config = await resolveConfig();
        assert.equal(config.repository, "ubiquity-os/test-plugin");
        assert.equal(config.refName, "feature/fallback");
      }
    );
  });
});
